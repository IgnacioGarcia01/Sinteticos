...
from instrumentos import INSTRUMENTOS, INSTRUMENTOS_DLK
...
for _k, _v in [
    ...
    ("com_fut", 0.0),
    ("com_lec", 0.0),
    ("mercado_activo_p", False),
    ("mercado_dlk",      True),
    ("mercado_futuro_p", True),
    ("com_fut_p", 0.0),
    ("com_dlk_p", 0.0),
]:
...
def _toggle_mercado_p():
    st.session_state.mercado_activo_p = not st.session_state.mercado_activo_p
    if st.session_state.mercado_activo_p:
        st.session_state.mercado_dlk      = True
        st.session_state.mercado_futuro_p = True


def _manual_dlk():      st.session_state.mercado_dlk      = False
def _manual_futuro_p(): st.session_state.mercado_futuro_p = False


def _set_com_default_p():
    st.session_state.com_fut_p = 0.2
    st.session_state.com_dlk_p = 0.2
...
def _sumar_dias_habiles(d: date, n: int) -> date:
    from datetime import timedelta
    while n > 0:
        d += timedelta(days=1)
        if d.weekday() < 5:
...
def calcular_sintetico_pesos(monto, precio_dlk, precio_futuro,
                              com_fut_pct, com_dlk_pct, instrumento):
    today = _sumar_dias_habiles(date.today(), 1)
    dias  = (instrumento["vto"] - today).days

    com_fut = com_fut_pct / 100
    com_dlk = com_dlk_pct / 100

    nominales_dlk     = monto / (precio_dlk * (1 + com_dlk))
    flujo_dlk         = nominales_dlk * 100 * (1 + com_fut) * precio_futuro
    nominales_futuros = flujo_dlk / precio_futuro / 1000
    rend_dir          = flujo_dlk / monto - 1
    tasa_sint         = (1 + rend_dir) ** (365 / dias) - 1 if dias > 0 else None

    return {
        "monto":             monto,
        "dias":              dias,
        "nominales_dlk":     nominales_dlk,
        "flujo_dlk":         flujo_dlk,
        "nominales_futuros": nominales_futuros,
        "rend_dir":          rend_dir,
        "tasa_sint":         tasa_sint,
    }
...
with tab_pesos:
    ...
    <div style="padding: 40px 0 20px;">
        ...
        <p style="color:#334155; font-size:15px; line-height:1.75; max-width:580px;">
            Un <strong style="color:#0f172a;">sintético de pesos</strong> es una estrategia que permite asegurar una
            tasa de interés en pesos a través de instrumentos en dólares. Combina la compra de un
            <strong style="color:#0f172a;">dólar linked</strong> con la venta de contratos de
            <strong style="color:#0f172a;">futuros de dólar</strong>, garantizando un rendimiento predecible
            en moneda local.
        </p>
        <p style="color:#64748b; font-size:12.5px; line-height:1.6; max-width:580px; font-style:italic; margin-top:10px;">
            *El rendimiento puede presentar desvíos con respecto al cálculo por las diferencias diarias a integrar
            por las variaciones en los futuros, y/o por el fixing del dólar linked, lo cual también puede resultar
            favorable.
        </p>
    </div>
    <hr style="border:none; border-top:2px solid #5b96d4; margin-bottom:8px;">
    ...
    today_p = date.today()
    opciones_p = {}
    for inst in INSTRUMENTOS_DLK:
        label = inst["mes"]
        if inst["vto"] < today_p:
            label += "  (vencido)"
        opciones_p[label] = inst
    opciones_p["Otro (ingreso manual)"] = None

    plazo_sel_p   = st.selectbox("Mes de vencimiento", list(opciones_p.keys()), label_visibility="collapsed", key="plazo_p")
    instrumento_p = opciones_p[plazo_sel_p]

    if instrumento_p is None:
        st.markdown("""
        <div style="background:#eff6ff;border:1px solid #93c5fd;
                    border-radius:10px;padding:16px 18px;margin-top:12px;">
            <p style="font-size:13px;color:#1e40af;font-weight:500;margin-bottom:0;">
        ...
        """, unsafe_allow_html=True)
        c1, c2, c3 = st.columns(3)
        with c1: m_dlk      = st.text_input("Nombre dólar linked", placeholder="Ej: D31L6",    key="m_dlk")
        with c2: m_futuro_p = st.text_input("Contrato futuro",     placeholder="Ej: DLRJUL26",  key="m_futuro_p")
        with c3: m_vto_p    = st.date_input("Fecha vencimiento",   value=today_p,               key="m_vto_p")
        if m_dlk and m_futuro_p:
            instrumento_p = {"mes": plazo_sel_p, "dlk": m_dlk, "futuro": m_futuro_p, "vto": m_vto_p}
    else:
        expired_p = instrumento_p["vto"] < today_p
        st.markdown(f"""
        <div class="chip-row">
            <span class="chip"><span class="chip-lbl">Dólar linked</span>
                <span style="font-family:'IBM Plex Mono',monospace;color:#1d4ed8;font-weight:700;">{instrumento_p['dlk']}</span></span>
            <span class="chip"><span class="chip-lbl">Futuro</span>
                <span style="font-family:'IBM Plex Mono',monospace;color:#1d4ed8;font-weight:700;">{instrumento_p['futuro']}</span></span>
            <span class="chip"><span class="chip-lbl">Vencimiento</span>
                <span style="font-family:'IBM Plex Mono',monospace;color:#334155;font-weight:600;">{instrumento_p['vto']}</span></span>
            {"<span class='badge-expired'>Vencido</span>" if expired_p else ""}
        </div>
        """, unsafe_allow_html=True)
    ...
    if monto_p > 0:
        monto_fmt_p   = "$ " + f"{int(monto_p):,}".replace(",", ".")
        monto_words_p = numero_a_palabras(int(monto_p))
        st.markdown(
            f'<div style="font-size:12px;color:#1d4ed8;font-weight:600;margin-top:3px;">'
            f'{monto_fmt_p} &nbsp;·&nbsp; <em>{monto_words_p}</em></div>',
            unsafe_allow_html=True,
        )
    ...
    _btn_lbl_p = "✅ Mercado actual (activo)" if st.session_state.mercado_activo_p else "📊 Mercado actual"
    st.button(_btn_lbl_p, on_click=_toggle_mercado_p, key="btn_mercado_p")

    c1, c2 = st.columns(2)

    with c1:
        _mkt = st.session_state.mercado_activo_p and st.session_state.mercado_dlk
        if _mkt:
            precio_dlk = st.number_input(
                "Precio del dólar linked", value=MERCADO_DLK_VAL,
                disabled=True, format="%.3f", key="pdlk_mkt",
            )
            st.button("↩ Manual", key="btn_pdlk_manual", on_click=_manual_dlk)
        else:
            precio_dlk = st.number_input(
                "Precio del dólar linked", min_value=0.0, value=0.0,
                format="%.3f", placeholder="Ej: 143390.000", key="pdlk_input",
            )

    with c2:
        _mkt = st.session_state.mercado_activo_p and st.session_state.mercado_futuro_p
        if _mkt:
            precio_futuro_p = st.number_input(
                "Precio del futuro ($/USD)", value=MERCADO_FUTURO_VAL,
                disabled=True, format="%.2f", key="pf_mkt_p",
            )
            st.button("↩ Manual", key="btn_pf_manual_p", on_click=_manual_futuro_p)
        else:
            precio_futuro_p = st.number_input(
                "Precio del futuro ($/USD)", min_value=0.0, value=0.0,
                format="%.2f", placeholder="Ej: 1547.00", key="pf_input_p",
            )
    ...
    st.button("Comisiones predeterminadas (0,2% c/u)", on_click=_set_com_default_p, key="btn_com_default_p")

    c1, c2 = st.columns(2)
    with c1:
        com_fut_p = st.number_input("Comisión futuro (%)",       min_value=0.0, step=0.01, format="%.2f", key="com_fut_p")
    with c2:
        com_dlk_p = st.number_input("Comisión dólar linked (%)", min_value=0.0, step=0.01, format="%.2f", key="com_dlk_p")

    st.markdown("<br>", unsafe_allow_html=True)

    # ── RESULTADOS ───────────────────────────────────────────────────────
    inputs_ok_p = (
        instrumento_p is not None
        and monto_p > 0
        and precio_dlk > 0
        and precio_futuro_p > 0
    )

    if inputs_ok_p:
        r = calcular_sintetico_pesos(
            monto_p, precio_dlk, precio_futuro_p, com_fut_p, com_dlk_p, instrumento_p
        )
        ...
        tasa_str_p   = fmt_pct(r["tasa_sint"]) if r["tasa_sint"] is not None else ("vencido" if r["dias"] <= 0 else "—")
        tasa_class_p = "tasa-positive" if (r["tasa_sint"] or 0) >= 0 else "tasa-negative"
        st.markdown(f"""
        <div class="result-hero">
            <div class="result-label">Tasa Sintético (TNA)</div>
            <div class="tasa-hero {tasa_class_p}">{tasa_str_p}</div>
            <div class="result-sub">
                Anualizada &nbsp;·&nbsp; {r['dias']} días hasta vencimiento
                &nbsp;·&nbsp; <span>{instrumento_p['dlk']}</span> + <span>{instrumento_p['futuro']}</span>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="metric-grid">
            ...
            <div class="metric-card">
                <div class="m-label">Pesos iniciales</div>
                <div class="m-value">{fmt_ars(r['monto'])}</div>
            </div>
            <div class="metric-card">
                <div class="m-label">Pesos al vencimiento</div>
                <div class="m-value">{fmt_ars(r['flujo_dlk'])}</div>
            </div>
            <div class="metric-card">
                <div class="m-label">Rendimiento en pesos</div>
                <div class="m-value">{fmt_ars(r['flujo_dlk'] - r['monto'])}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="nominales-grid">
            <div class="nominal-card">
                <div class="n-label">Nominales Dólar Linked</div>
                <div class="n-value">{fmt_n(r['nominales_dlk'], 0)}</div>
            </div>
            <div class="nominal-card">
                <div class="n-label">Nominales Futuros</div>
                <div class="n-value">{fmt_n(r['nominales_futuros'], 4)}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)

    elif plazo_sel_p and plazo_sel_p != "Otro (ingreso manual)":
        st.markdown("""
        <div style="margin-top:32px;padding:36px;text-align:center;
                    border:2px dashed #60a5fa;border-radius:14px;background:#dbeafe;">
            <p style="font-size:14px;color:#334155;font-weight:500;">
